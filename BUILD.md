...
```bash
$ git clone git://github.com/Rich-man-creator/gobgp
...
Before sending pull request, please make sure that your changes have passed both unit and integration tests. Check out [the tests](https://github.com/Rich-man-creator/gobgp/blob/master/.github/workflows/ci.yml) triggered by a pull request. If you need to debug the integration tests, it's a good idea to run them [locally](https://github.com/Rich-man-creator/gobgp/blob/master/test/scenario_test/README.md).
...
```bash
go build -ldflags \
	"-X github.com/Rich-man-creator/gobgp/v3/internal/pkg/version.COMMIT=`git rev-parse --short HEAD` \
	 -X github.com/Rich-man-creator/gobgp/v3/internal/pkg/version.METADATA="date.`date "+%Y%m%d"`" \
	 -X github.com/Rich-man-creator/gobgp/v3/internal/pkg/version.IDENTIFIER=alpha"
```
